...
- 一把 `TYPESAFE_API_KEY`（[TypeSafe](https://docs.typesafe.ai/introduction)，付費；上次抽三部劇花了 US$0.00175）
...
- 它用的是你自己的 Chrome，你登入的網站它也是登入狀態。
- 連不上 Chrome 時，執行 `uv run browser-harness --doctor`。
- 出現 `STOPPED: the LinkedIn window needs a human`，就在跳出的 LinkedIn 視窗手動登入，再跑一次。
...
最簡單的方法：把這份文件丟給 Claude Code 或 Codex，請它照著幫你裝好、跑起來。
